...
def get_labels_from_generator(generator):
    """
    Получить метки состояний для обучения из генератора сигнала
    """
    labels = list(map(rename_state,generator.path))
    return labels
def rename_path_for_train(path):
    return list(map(rename_state,path))
def get_y_train(x):
    y = x.copy()
...
def print_model_distribution(model):
    ...
    sample, path = model.sample( length = 10000, path=True )
