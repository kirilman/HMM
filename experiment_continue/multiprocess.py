...
if __name__ ==  "__main__":
    ...
    norm_params = {'a': {'len': [20, 20], 'depend_on': False},
                   'b': {'len': [100, 100], 'depend_on': False},
                   'c': {'len': [50, 50],'depend_on': False}}

    an_params =   {'a': {'len': [20, 21], 'depend_on': False},
                   'b': {'len': [80, 90], 'depend_on': False},
                   'c': {'len': [100,200], 'depend_on': False}}

    alpha = ['a','b','c']

    mean = [0, 2, 4]  ; variance = [0.01, 0.5, 0.01] + np.array([0.05]*3)
    anomal_mean = mean  
    anomal_mean = mean + np.array([0,0.00,0]) 
    ...
    model = HiddenMarkovModel.from_samples(NormalDistribution, n_components = n_comp, X = [normal_signal], n_jobs = 4)
