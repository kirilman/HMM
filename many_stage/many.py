...
def run_test(arg, k):
    ...
    #                           labels = [labels], algorithm = algorithm, stop_threshold = 1e-20)
...
if __name__ == "__main__":
    ...
    for i in range(3):
        ...
        signals +=[signal]
        models += [model]
    #Сохранить сигналы для обучения
    

    # model.plot(node_size = 100, with_labels = True,node_shape ='>')
    print()
    ...
    with open('result.txt', 'w') as file:
        for arg in args:
            ...
            for i in range(1):
                ...
                paths = [model.viterbi(test_signal)[1] for model in models]
                for x, path in zip(log_pr,paths):
                    file.write('{:.3f}    '.format(x))
                    w_path = [str(p[0]) for p in path]
                    # file.write(str(w_path)+'\n')

                file.write(str(np.argmax(log_pr) + 1)+'\n')
            # file.write('____\n')
        for model in models:
            s = myutils.print_model_distribution(model)
            file.write(s)
